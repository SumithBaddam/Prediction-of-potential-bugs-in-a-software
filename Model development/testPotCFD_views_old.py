# python ./testPotCFD_views_old.py --env Prod --bu ENB --queryID 3
# python ./testPotCFD_views_old.py --env Prod --viewID 673 --queryID 2061
...
def parse_options():
    ...
    parser.add_argument("--bu", default="", help='BU name, applicable for User defined queries', type=str, metavar='v')
    ...
    parser.add_argument("--cutOff", default="", help='CQI Query ID', type=str, metavar='q')
...
def main():
    ...
    if not options.queryID:
        ...
        queries = get_queries(db, settings, options)
        #print(queries)
        for q in queries:
            options.bu = str(q["bu_name"])
            options.queryID = int(q["query_id"])
            options.viewID = int(q["view_id"])
            test_view(db, options.viewID, options.queryID, options.bu, True, options)
        ...
        for q in queries:
            ...
            options.queryID = int(q["query_id"])
            options.viewID = int(q["view_id"])
            test_view(db, options.viewID, options.queryID, options.bu, False, options)
